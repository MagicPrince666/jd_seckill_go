...
func (this *Seckill) SeckillInitInfo() (string, error) {
	...
	req.SetHeader("User-Agent", this.conf.Read("config", "DEFAULT_USER_AGENT"))
	req.SetHeader("Host", "marathon.jd.com")
	bd := body.NewUrlEncode()
	bd.SetData("sku", skuId)
	bd.SetData("num", seckillNum)
	bd.SetData("isModifyAddress", "false")
	resp, body, err := req.SetBody(bd).SetUrl("https://marathon.jd.com/seckillnew/orderService/pc/init.action").SetMethod("post").Send().End()
	if err != nil || resp.StatusCode != http.StatusOK {
...
func (this *Seckill) SubmitSeckillOrder() bool {
	...
	bd := body.NewUrlEncode()
	bd.SetData("skuId", skuId)
	bd.SetData("num", seckillNum)
	bd.SetData("addressId", defaultAddress.Get("id").String())
	bd.SetData("yuShou", "true")
	bd.SetData("isModifyAddress", "false")
	bd.SetData("name", defaultAddress.Get("name").String())
	bd.SetData("provinceId", defaultAddress.Get("provinceId").String())
	bd.SetData("cityId", defaultAddress.Get("cityId").String())
	bd.SetData("countyId", defaultAddress.Get("countyId").String())
	bd.SetData("townId", defaultAddress.Get("townId").String())
	bd.SetData("addressDetail", defaultAddress.Get("addressDetail").String())
	bd.SetData("mobile", defaultAddress.Get("mobile").String())
	bd.SetData("mobileKey", defaultAddress.Get("mobileKey").String())
	bd.SetData("email", defaultAddress.Get("email").String())
	bd.SetData("postCode", "")
	bd.SetData("invoiceTitle", invoiceTitle)
	bd.SetData("invoiceCompanyName", "")
	bd.SetData("invoiceContent", invoiceContentType)
	bd.SetData("invoiceTaxpayerNO", "")
	bd.SetData("invoiceEmail", "")
	bd.SetData("invoicePhone", invoicePhone)
	bd.SetData("invoicePhoneKey", invoicePhoneKey)
	bd.SetData("invoice", invoiceInfo)
	bd.SetData("password", paymentPwd)
	bd.SetData("codTimeType", "3")
	bd.SetData("paymentType", "4")
	bd.SetData("areaCode", "")
	bd.SetData("overseas", "0")
	bd.SetData("phone", "")
	bd.SetData("eid", eid)
	bd.SetData("fp", fp)
	bd.SetData("token", token)
	bd.SetData("pru", "")
	resp, body, err := req.SetBody(bd).SetUrl("https://marathon.jd.com/seckillnew/orderService/pc/submitOrder.action?skuId=" + skuId).SetMethod("post").Send().End()
	if err != nil || resp.StatusCode != http.StatusOK {
